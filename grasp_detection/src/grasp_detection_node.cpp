...
{   
  ...
  cloud_.reset(new pcl::PointCloud<pcl::PointXYZ>);
  cloud_->header.frame_id = "camera_link";
  pcl::fromPCLPointCloud2( *cloud2, *cloud_);
  //Eigen::Affine3f transform_pc = Eigen::Affine3f::Identity();
  //transform_pc.translation() << 0.045, 0.85, -1.31;
  //pcl::transformPointCloud (*cloud_, *cloud_, transform_pc);
  //transform_pc = Eigen::Affine3f::Identity();
  //transform_pc.rotate (Eigen::AngleAxisf (-0.745, Eigen::Vector3f::UnitY()));
  //transform_pc.rotate (Eigen::AngleAxisf (-1.56, Eigen::Vector3f::UnitZ()));
  //pcl::transformPointCloud (*cloud_, *cloud_, transform_pc);
